...
namespace AStarPairsParallel
{

constexpr unsigned long long NT = 16;
...
struct Vec3
{
    ...
    Vec3(const AStarPairsParallel::Vec3<real>& V) : x(V.x), y(V.y), z(V.z) { }

    real dist2(const AStarPairsParallel::Vec3<real>& V) const
    {
        double xx = x - V.x;
        double yy = y - V.y;
        double zz = z - V.z;
    ...
    real dist(const AStarPairsParallel::Vec3<real>& V) const { return std::sqrt(dist2(V)); }
};
...
struct QueueGreater
{
    constexpr bool operator()(const AStarPairsParallel::QueueEntry<real>& a, 
                              const AStarPairsParallel::QueueEntry<real>& b)
    {
        if (a.Guess > b.Guess)
            return true;
        if (b.Guess > a.Guess)
...
{
private:
    std::vector<AStarPairsParallel::Vec3<real>> m_Pos;
...
public:
    ...
    real Distance(int i, int j) const
    {
        ...
        std::priority_queue<AStarPairsParallel::QueueEntry<real>, 
                            std::vector<AStarPairsParallel::QueueEntry<real>>, 
                            AStarPairsParallel::QueueGreater<real>> pq;
        dists[i] = 0.0;
...
public:
                    ...
                    matlab::mex::ArgumentList inputs)
    {
        ...
        if (inputs[0].getType() == matlab::data::ArrayType::DOUBLE)
        {
            ...
            AStarPairsParallel::Graph<double> G(verts, inputs[0].getDimensions()[1], edges, inputs[1].getDimensions()[1]);
            ...
            auto dlambda = [&](unsigned long long beg, unsigned long long end) { 
                for (unsigned long long i = beg; i < end; ++i)
                    outputs[0][i] = G.Distance(pairs[2 * i], pairs[2 * i + 1]);
            };
            std::vector<unsigned long long> Idx;
            for (unsigned long long i = 0; i < AStarPairsParallel::NT; ++i)
                Idx.emplace_back(i * (npairs / AStarPairsParallel::NT));
            Idx.emplace_back(npairs);
            std::vector<std::thread> Threads;
            for (unsigned long long i = 0; i < AStarPairsParallel::NT; ++i)
                Threads.emplace_back(dlambda, Idx[i], Idx[i + 1]);
            for (unsigned long long i = 0; i < AStarPairsParallel::NT; ++i)
                Threads[i].join();
        ...
        else
        {
            ...
            AStarPairsParallel::Graph<float> G(verts, inputs[0].getDimensions()[1], edges, inputs[1].getDimensions()[1]);
